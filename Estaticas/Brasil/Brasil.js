...
async function fetchDataFromAPIPrice() {
    try {
        const response = await fetch('https://32tpwbxjq7.us-east-1.awsapprunner.com/api/landing-brasils?populate=*');
        if (!response.ok) {
            throw new Error('No se pudo obtener los datos de la API');
        }
            ...
            try {
                ...
                const prices = responseData.data.reduce((acc, item) => {
                    ...
                    acc[destino][card].push({
                        ...
                        Imagen_Card: item?.attributes?.Imagen_Card[0]?.data?.attributes?.url
                    });

                    console.log("Datos de la tarjeta:", acc[destino][card]);
                    console.log("URL de la Imagen_Card[0] de la tarjeta:", item?.attributes?.Imagen_Card[0]?.data?.attributes?.url);
